...
class SessionBuilder {
    constructor(storage, remoteAddress, logger) {
        this.processPreKeyJob = async (device) => {
            // Trust is checked before bundle signature verification so persisted identity decisions win.
            const trusted = await this.storage.isTrustedIdentity(this.getRemoteIdentityIdentifier(), device.identityKey, types_1.Direction.SENDING);
            if (!trusted) {
            ...
            await Internal.crypto.Ed25519Verify(device.identityKey, device.signedPreKey.publicKey, device.signedPreKey.signature); // This will throw if invalid
            ...
            const session = await this.startSessionAsInitiator(ephemeralKey, device.identityKey, device.signedPreKey.publicKey, deviceOneTimePreKey, device.registrationId);
        ...
        this.startSessionAsInitiator = async (EKa, IKb, SPKb, OPKb, registrationId) => {
            ...
            const masterKey = await Internal.HKDF((0, helpers_1.uint8ArrayToArrayBuffer)(sharedSecret), new ArrayBuffer(32), 'WhisperText');
            const session = {
        ...
        this.startSessionWthPreKeyMessage = async (OPKb, SPKb, message) => {
            ...
            const IKa = (0, helpers_1.uint8ArrayToArrayBuffer)(message.identityKey);
            const EKa = (0, helpers_1.uint8ArrayToArrayBuffer)(message.baseKey);
            if (!IKb) {
    ...
    getRemoteIdentityIdentifier() {
        return typeof this.remoteAddress.getName === 'function'
    ...
    async processPreKey(device) {
        ...
        return session_lock_1.SessionLock.queueJob(this.remoteAddress.toString(), runJob);
    }
    async processV3(record, message) {
        const trusted = await this.storage.isTrustedIdentity(this.getRemoteIdentityIdentifier(), (0, helpers_1.uint8ArrayToArrayBuffer)(message.identityKey), types_1.Direction.RECEIVING);
        if (!trusted) {
            this.logger.sendEvent(`1-1-create-receiver-session:address=${this.remoteAddress.toString()}`, {
                functionName: 'processV3',
                error: `Unknown identity key: ${(0, helpers_1.uint8ArrayToArrayBuffer)(message.identityKey)}`,
            });
        ...
        if (record.getSessionByBaseKey((0, helpers_1.uint8ArrayToArrayBuffer)(message.baseKey))) {
            this.logger.sendEvent(`1-1-create-receiver-session:address=${this.remoteAddress.toString()}`, {
                functionName: 'processV3',
                info: `Found a session created already with baseKey: ${(0, helpers_1.arrayBufferToString)((0, helpers_1.uint8ArrayToArrayBuffer)(message.baseKey))} and the preKey used ${message.preKeyId}`,
            });
        ...
        await this.storage.saveIdentity(this.remoteAddress.toString(), (0, helpers_1.uint8ArrayToArrayBuffer)(message.identityKey));
        return message.preKeyId;
    }
}
